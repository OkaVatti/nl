...
module Nl
  ...
  class Socket
    ...
    # Receives a single netlink message.
    #
    # Returns a new `Nl::Message` object containing the received data.
    def recv : Message
      buf_size = msg_buf_size
      # If the buffer size is 0, use a sane default (like 8192)
      buf_size = 8192 if buf_size == 0
      buf = Bytes.new(buf_size)
      ret = LibNL.nl_recv(@ptr, buf, buf_size)
      raise Error.from_ret(ret) if ret < 0
      # ret is the number of bytes received; the buffer contains the netlink header.
      # Convert the buffer to a nl_msg object using nlmsg_convert.
      msg_ptr = LibNL.nlmsg_convert(buf.to_unsafe)
      raise Error.new("Failed to convert netlink message") if msg_ptr.null?
      Message.new(msg_ptr, owned: true)
    end
